...
<div class="container mx-left max-w-md p-6">
    <h1 class="text-2xl font-semibold">Category Edit</h1>
    <form method="POST" action="{{route('category.update',$category)}}" enctype="multipart/form-data" class="mt-4">
        @csrf

        <div class="mb-4 mt-10">
            <label for="category" class="block text-sm font-medium text-gray-700">Category Name</label>
            <input type="text" name="category" id="category" class="mt-1 p-2 w-full rounded border border-gray-300 focus:outline-none focus:ring focus:ring-blue-200" value="{{$category->category}}">
        </div>
        ...
        <div class="mb-4 mt-10">
            ...
            <div class="mt-2 space-y-2">
                <label for="status_active" class="inline-flex items-center">
                    <input type="checkbox" name="status" id="status_active" class="form-checkbox text-blue-600 focus:ring focus:ring-blue-200" value="1" {{ $category->status == 1 ? 'checked' : '' }}>
